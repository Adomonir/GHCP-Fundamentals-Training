...
def sample_files(temp_dir):
    """Create sample files with globex patterns."""
    # Create Python file with globex content
...
def test_rename_files(sample_files):
    """Test that files with globex in name are renamed."""
...
def test_rename_file_content(sample_files):
    """Test that file contents are updated with globex → chroma."""
    rename_log, stats = rename_globex_to_chroma(sample_files)
...
def test_skip_binary_files(sample_files):
    """Test that binary files are renamed but content is not processed."""
    bin_file_old = Path(sample_files) / "globex_image.bin"
    original_content = bin_file_old.read_bytes()
    
    rename_log, stats = rename_globex_to_chroma(sample_files)
    
    # Binary file should be renamed
    bin_file_new = Path(sample_files) / "chroma_image.bin"
    assert bin_file_new.exists()
    assert not bin_file_old.exists()
    # Content should remain unchanged
    assert bin_file_new.read_bytes() == original_content
...
def test_multiple_replacements_in_file(temp_dir):
    """Test file with multiple globex occurrences."""
